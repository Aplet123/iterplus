...
export class AsyncIterPlus<T> implements CurIter<T>, AsyncIterable<T> {
     ...
     */
    async group<K extends string | number | symbol>(
        key: (elem: T) => PromiseOrValue<K>
    ): Promise<Record<K, T[]>> {
     ...
     */
    async tallyWith<K extends string | number | symbol>(
        key: (elem: T) => PromiseOrValue<K>
     ...
     */
    async tally(
        this: AsyncIterPlus<string | number | symbol>
    ...
    globBy(
        ...
        async function* ret() {
            ...
            for await (const elem of that) {
                if (
                    curGlob.length === 0 ||
                    (await cmp(curGlob[curGlob.length - 1], elem))
                ) {
                    curGlob.push(elem);
                } else {
                    yield curGlob;
